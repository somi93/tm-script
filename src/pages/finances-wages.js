...
const render = (main, sourceRoot) => {
    ...
    const tabsData = parseTabData(sourceRoot);
...
export function initFinancesWagesPage(main) {
    const sourceRoot = document.querySelector('.main_center');
    if (!main || !sourceRoot) return;
    if (!sourceRoot.querySelector('.column1 .content_menu, #tab0 table, #tab1 table')) return;
