...
` }));
  ...
  (function() {
    ...
    const STYLE_ID3 = "tmvu-finances-style";
    const cleanText = (value) => String(value || "").replace(/\s+/g, " ").trim();
    const escapeHtml3 = (value) => String(value || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
    ...
    const iconForMenu = (label) => {
      ...
      if (/maintenance/i.test(label)) return "\u{1F3D7}";
      if (/wages/i.test(label)) return "\u{1F4B8}";
      if (/players/i.test(label)) return "\u{1F465}";
    ...
    const parseStatement = (table) => {
      ...
      const rows = Array.from(table.querySelectorAll("tr")).slice(1).map((row) => {
        ...
        const current = parseMoney(((_b2 = cells[1]) == null ? void 0 : _b2.textContent) || "0");
        const previous = parseMoney(((_c2 = cells[2]) == null ? void 0 : _c2.textContent) || "0");
        return {
          label,
          tooltip,
          current,
      ...
      return {
        columns: [
          cleanText(((_a = headerCells[0]) == null ? void 0 : _a.textContent) || ""),
          cleanText(((_b = headerCells[1]) == null ? void 0 : _b.textContent) || "Current"),
          cleanText(((_c = headerCells[2]) == null ? void 0 : _c.textContent) || "Previous")
        ],
        rows,
    ...
    const parseOverview = () => {
      var _a, _b, _c, _d;
      const title = cleanText(((_a = sourceRoot.querySelector(".column2_a .box_head h2")) == null ? void 0 : _a.textContent) || "Finances");
      const balanceText = ((_b = sourceRoot.querySelector(".column2_a .very_large .coin_big")) == null ? void 0 : _b.textContent) || "0";
      const pendingText = ((_d = (_c = Array.from(sourceRoot.querySelectorAll(".column2_a .align_center")).find((node) => /pending transfers/i.test(node.textContent || ""))) == null ? void 0 : _c.querySelector(".coin")) == null ? void 0 : _d.textContent) || "";
      return {
        title,
    ...
    const buildStatCardsHtml = (overview) => {
      ...
      return `
            <div class="tmvu-fin-stat-grid">
                ${metricHtml({ label: "Weekly Net", value: escapeHtml3(formatSignedMoney(weekTotal)), note: `Compared with ${escapeHtml3(formatMoney((_g = (_f = week == null ? void 0 : week.total) == null ? void 0 : _f.previous) != null ? _g : 0))} last week.`, tone: "overlay", size: "md", valueCls: deltaClass(weekTotal) })}
                ${metricHtml({ label: "Season Net", value: escapeHtml3(formatSignedMoney(seasonTotal)), note: `Compared with ${escapeHtml3(formatSignedMoney((_i = (_h = season == null ? void 0 : season.total) == null ? void 0 : _h.previous) != null ? _i : 0))} last season.`, tone: "overlay", size: "md", valueCls: deltaClass(seasonTotal) })}
                ${metricHtml({ label: "Pending Transfers", value: escapeHtml3(formatSignedMoney(pending)), note: "Queued movements that have not hit the balance yet.", tone: "overlay", size: "md", valueCls: deltaClass(pending) })}
            </div>
        `;
    };
    const renderHeroCard = (overview) => {
      ...
      TmHeroCard.mount(wrap, {
        ...
        slots: {
          kicker: "Finances",
          title: escapeHtml3(overview.title),
          main: `
                    <div class="tmvu-fin-hero-metrics">
                        ${metricHtml({ label: "Current Balance", value: escapeHtml3(formatSignedMoney(overview.balance)), tone: "overlay", size: "lg" })}
                        ${hasValue(overview.pending) ? metricHtml({ label: "Pending Transfers", value: escapeHtml3(formatSignedMoney(overview.pending)), tone: "overlay", size: "md", valueCls: deltaClass(overview.pending) }) : ""}
                    </div>
                `,
      ...
      return `
            <div class="tmvu-fin-table-wrap">
                <table class="tmvu-fin-table">
                    <thead>
                        <tr>
                            <th>${escapeHtml3(statement.columns[0] || "")}</th>
                            <th>${escapeHtml3(statement.columns[1] || "Current")}</th>
                            <th>${escapeHtml3(statement.columns[2] || "Previous")}</th>
                            <th>Delta</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${statement.rows.map((row) => `
                            <tr class="${row.isTotal ? "tmvu-fin-total" : ""}">
                                <th>
                                    <span class="tmvu-fin-label"${row.tooltip ? ` title="${escapeHtml3(row.tooltip)}"` : ""}>
                                        <span class="tmvu-fin-label-dot"></span>
                                        <span>${escapeHtml3(row.label)}</span>
                                    </span>
                                </th>
                                <td>${escapeHtml3(formatSignedMoney(row.current))}</td>
                                <td>${escapeHtml3(formatSignedMoney(row.previous))}</td>
                                <td><span class="tmvu-fin-delta ${deltaClass(row.delta)}">${escapeHtml3(formatSignedMoney(row.delta))}</span></td>
                            </tr>
                        `).join("")}
                    </tbody>
                </table>
    ...
    const renderTableCard = (overview) => {
      ...
      const refs = TmUI.render(wrap, `
            <tm-card data-title="Cashflow" data-icon="\u{1F4D1}">
    ...
    const renderHighlightsCard = (title, summary, previousLabel) => {
      ...
      const wrap = document.createElement("section");
      TmUI.render(wrap, `
            <tm-card data-title="${escapeHtml3(title)}" data-icon="\u{1F4C8}">
                <div class="tmvu-fin-highlights">
                    ${metricHtml({ label: "Net Result", value: escapeHtml3(formatSignedMoney((_b = (_a = summary == null ? void 0 : summary.total) == null ? void 0 : _a.current) != null ? _b : 0)), note: `Reference ${escapeHtml3(previousLabel)}: ${escapeHtml3(formatSignedMoney((_d = (_c = summary == null ? void 0 : summary.total) == null ? void 0 : _c.previous) != null ? _d : 0))}`, tone: "overlay", size: "sm", valueCls: deltaClass((_f = (_e = summary == null ? void 0 : summary.total) == null ? void 0 : _e.current) != null ? _f : 0) })}
                    ${metricHtml({ label: "Largest Income", value: escapeHtml3(((_g = summary == null ? void 0 : summary.bestIncome) == null ? void 0 : _g.label) || "None"), note: escapeHtml3(formatSignedMoney((_i = (_h = summary == null ? void 0 : summary.bestIncome) == null ? void 0 : _h.current) != null ? _i : 0)), tone: "overlay", size: "sm" })}
                    ${metricHtml({ label: "Heaviest Cost", value: escapeHtml3(((_j = summary == null ? void 0 : summary.biggestCost) == null ? void 0 : _j.label) || "None"), note: escapeHtml3(formatSignedMoney((_l = (_k = summary == null ? void 0 : summary.biggestCost) == null ? void 0 : _k.current) != null ? _l : 0)), tone: "overlay", size: "sm" })}
                </div>
            </tm-card>
        `);
      return wrap.firstElementChild || wrap;
      ...
      TmUI.render(wrap, `
            <tm-card data-title="Cash Position" data-icon="\u{1F3E6}">
                <div class="tmvu-fin-balance">
                    ${metricHtml({ label: "Current Balance", value: escapeHtml3(formatSignedMoney(overview.balance)), layout: "row", tone: "muted", size: "lg" })}
                    ${hasValue(overview.pending) ? `
                        ${metricHtml({ label: "Pending Transfers", value: escapeHtml3(formatSignedMoney(overview.pending)), layout: "row", tone: "muted", size: "sm", valueCls: deltaClass(overview.pending) })}
                    ` : ""}
                </div>
            </tm-card>
        `);
    ...
    const render = () => {
      ...
      sideCol.appendChild(renderHighlightsCard("Week Snapshot", weekSummary, ((_b = (_a = overview.week) == null ? void 0 : _a.columns) == null ? void 0 : _b[2]) || "last week"));
      sideCol.appendChild(renderHighlightsCard("Season Snapshot", seasonSummary, ((_d = (_c = overview.season) == null ? void 0 : _c.columns) == null ? void 0 : _d[2]) || "last season"));
