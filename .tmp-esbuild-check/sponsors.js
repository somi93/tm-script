...
` }));
  ...
  (function() {
    ...
    const STYLE_ID3 = "tmvu-sponsors-style";
    let refreshTimer = 0;
    ...
    const titleize = (value) => cleanText(value).replace(/[_-]+/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
    ...
    const getLabelForInput = (root, input) => {
      var _a;
      if (!root || !(input == null ? void 0 : input.id)) return null;
    ...
    const extractOptionNote = (row, optionLabel, bonusText) => {
      if (!row) return "";
      const texts = Array.from(row.querySelectorAll("th, td")).map((cell) => cleanText(cell.textContent)).filter(Boolean).filter((text) => text !== optionLabel && text !== bonusText);
      return texts.join(" | ");
    };
      ...
      return Array.from(box.querySelectorAll("h3")).map((heading) => {
        ...
        return {
          ...
          options: radios.map((radio) => {
            ...
            const optionLabel = cleanText((labelNode == null ? void 0 : labelNode.textContent) || radio.value || radio.name);
            const bonusNode = row == null ? void 0 : row.querySelector("td.align_right, .align_right");
            const bonusText = cleanText((bonusNode == null ? void 0 : bonusNode.textContent) || "0");
            ...
            return {
              ...
              imageSrc: (imageNode == null ? void 0 : imageNode.getAttribute("src")) || ""
            };
          })
        };
      }).filter(Boolean);
    ...
    const parseOfferState = (host) => {
      ...
      const offers = nodes.map((node) => {
        ...
        const title = cleanText(((_a = node.querySelector("h1, h2, h3, strong, b")) == null ? void 0 : _a.textContent) || "Sponsor Offer");
        const paragraphs = Array.from(node.querySelectorAll("p, li")).map((item) => cleanText(item.textContent)).filter(Boolean);
        ...
        const amount = parseMoney((moneyNode == null ? void 0 : moneyNode.textContent) || "0");
        ...
        return {
          ...
          meta,
          amount,
          imageSrc: (image == null ? void 0 : image.getAttribute("src")) || "",
          actions,
    ...
    const getViewState = () => {
      ...
      const selectedGoals = goalGroups.map((group) => group.options.find((option) => option.checked)).filter(Boolean);
    ...
    const renderHero = (state) => {
      ...
      const expiry = ((_a = state.primaryOffer) == null ? void 0 : _a.expires) || (state.offerState.status === "loading" ? "Refreshing..." : "-");
        ...
        slots: {
          ...
          footer: `
                    <div class="tmvu-fin-hero-metrics">
                        ${metricHtml({ label: "Offer Value", value: escapeHtml3(payout), note: escapeHtml3(((_b = state.primaryOffer) == null ? void 0 : _b.contract) || "Awaiting sponsor offer"), tone: "overlay", size: "lg" })}
                        ${metricHtml({ label: "Selected Bonuses", value: escapeHtml3(formatMoney(state.selectedBonusTotal)), tone: "overlay", size: "md" })}
                        ${metricHtml({ label: "Targets Active", value: escapeHtml3(String(state.selectedGoals.length)), note: escapeHtml3(expiry), tone: "overlay", size: "md" })}
                    </div>
                `
        }
      });
    ...
    const renderGoalsCard = (state) => {
      ...
      info.textContent = "Choose one target per group, then confirm any sponsor action through the hidden native TM flow.";
      ...
      state.goalGroups.forEach((group) => {
        ...
        header.innerHTML = `
                <div class="tmvu-sponsors-group-title">${escapeHtml3(group.title)}</div>
                <div class="tmvu-sponsors-group-meta">${escapeHtml3(selected ? formatMoney(selected.bonus) : "No bonus")}</div>
            `;
        ...
        group.options.forEach((option) => {
          ...
          button.innerHTML = `
                    <div class="tmvu-sponsors-option-media">
                        ${option.imageSrc ? `<img class="tmvu-sponsors-option-logo" src="${escapeHtml3(option.imageSrc)}" alt="">` : ""}
                        <div>
                            <div class="tmvu-sponsors-option-name">${escapeHtml3(option.name)}</div>
                            ${option.note ? `<div class="tmvu-sponsors-option-note">${escapeHtml3(option.note)}</div>` : ""}
                        </div>
                    </div>
                    <div class="tmvu-sponsors-option-bonus">${escapeHtml3(option.bonus ? formatMoney(option.bonus) : "-")}</div>
                `;
      ...
      state.offerState.offers.forEach((offer) => {
        ...
        offerEl.innerHTML = `
                <div class="tmvu-sponsors-offer-top">
                    <div>${offer.imageSrc ? `<img class="tmvu-sponsors-offer-image" src="${escapeHtml3(offer.imageSrc)}" alt="Sponsor">` : ""}</div>
                    <div>
                        <div class="tmvu-sponsors-offer-label">Live Sponsor Output</div>
                        <div class="tmvu-sponsors-offer-title">${escapeHtml3(offer.title || "Sponsor Offer")}</div>
                        ${offer.contract ? `<div class="tmvu-sponsors-offer-contract">${escapeHtml3(offer.contract)}</div>` : ""}
                        <div class="tmvu-sponsors-offer-amount">${escapeHtml3(formatMoney(offer.amount))}</div>
                    </div>
                </div>
            `;
        ...
        const metaItems = [
          { label: "Expires", value: offer.expires || "-" },
        ...
        metaItems.forEach((item) => {
          ...
          cell.innerHTML = `
                    <div class="tmvu-sponsors-offer-meta-name">${escapeHtml3(item.label)}</div>
                    <div class="tmvu-sponsors-offer-meta-value">${escapeHtml3(item.value)}</div>
                `;
    ...
    const renderSnapshotCard = (state) => {
      ...
      const items = [
        ...
        { label: "Expiry", value: ((_a = state.primaryOffer) == null ? void 0 : _a.expires) || "-" }
      ];
      items.forEach((item) => {
        const row = document.createElement("div");
        row.className = "tmvu-sponsors-summary-item";
        row.innerHTML = `
                <div class="tmvu-sponsors-summary-name">${escapeHtml3(item.label)}</div>
                <div class="tmvu-sponsors-summary-value">${escapeHtml3(item.value)}</div>
            `;
    ...
    const renderSelectedGoalsCard = (state) => {
      ...
      state.goalGroups.forEach((group) => {
        ...
        row.innerHTML = `
                <div>
                    <div class="tmvu-sponsors-summary-name">${escapeHtml3(group.title)}</div>
                    <div class="tmvu-sponsors-note">${escapeHtml3(selected.name)}</div>
                </div>
                <div class="tmvu-sponsors-summary-value">${escapeHtml3(formatMoney(selected.bonus))}</div>
            `;
        list.appendChild(row);
      });
      card.appendChild(list);
