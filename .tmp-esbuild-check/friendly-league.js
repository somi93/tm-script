...
` }));
  ...
  var STYLE_ID3 = "tmvu-round-navigator-style";
  ...
  function mount2(mainContainer, { id, className = "", items = [], currentHref = "" } = {}) {
  ...
  function parseStandingsRow(row, { highlightedClubId = "", rankFallback = 0 } = {}) {
    ...
    return {
      rank: hasRankColumn ? Number.parseInt(cleanText(((_a = cells[0]) == null ? void 0 : _a.textContent) || ""), 10) || rankFallback : rankFallback,
      clubId,
      clubName: cleanText(clubLink.textContent || ""),
      gp: Number.parseInt(cleanText(((_b = cells[1 + offset]) == null ? void 0 : _b.textContent) || ""), 10) || 0,
      w: Number.parseInt(cleanText(((_c = cells[2 + offset]) == null ? void 0 : _c.textContent) || ""), 10) || 0,
      d: Number.parseInt(cleanText(((_d = cells[3 + offset]) == null ? void 0 : _d.textContent) || ""), 10) || 0,
      l: Number.parseInt(cleanText(((_e = cells[4 + offset]) == null ? void 0 : _e.textContent) || ""), 10) || 0,
      gf: Number.parseInt(cleanText(((_f = cells[5 + offset]) == null ? void 0 : _f.textContent) || ""), 10) || 0,
      ga: Number.parseInt(cleanText(((_g = cells[6 + offset]) == null ? void 0 : _g.textContent) || ""), 10) || 0,
      pts: Number.parseInt(cleanText(((_h = cells[7 + offset]) == null ? void 0 : _h.textContent) || ""), 10) || 0,
  ...
  function isGroupTitleRow(row) {
    ...
    const firstLabel = cleanText(((_a = headers[0]) == null ? void 0 : _a.textContent) || "").toLowerCase();
    ...
    parseNativeGroupedTable(table, { highlightedClubId = "" } = {}) {
      ...
      Array.from((table == null ? void 0 : table.querySelectorAll("tr")) || []).forEach((row) => {
        ...
        if (isGroupTitleRow(row)) {
          ...
          currentGroup = { title: cleanText(((_a = row.querySelector("th")) == null ? void 0 : _a.textContent) || ""), rows: [] };
          return;
        }
        if (row.querySelector("th")) return;
        const parsed = parseStandingsRow(row, {
          highlightedClubId,
          rankFallback: ((currentGroup == null ? void 0 : currentGroup.rows.length) || 0) + 1
        });
        if (!parsed) return;
  ...
  var STYLE_ID5 = "tmvu-standings-table-style";
  var htmlOf3 = (node) => node ? node.outerHTML : "";
  var buttonHtml2 = (opts) => htmlOf3(TmButton.button(opts));
  ...
  var TmStandingsTable = { injectStyles: injectStyles5, buildHtml, buildGroupedHtml };
  ...
  (function() {
    ...
    const STYLE_ID6 = "tmvu-friendly-league-style";
    ...
    const cleanText2 = (value) => String(value || "").replace(/\s+/g, " ").trim();
    ...
    const parseMenu = () => Array.from(sourceRoot.querySelectorAll(".column1 .content_menu > *")).flatMap((node) => {
      ...
      const label = cleanText2(node.textContent);
      return [{
        type: "link",
        href: node.getAttribute("href") || "#",
        label,
        icon: /fixtures/i.test(label) ? "\u{1F4C5}" : /statistics/i.test(label) ? "\u{1F4CA}" : /history/i.test(label) ? "\u{1F4DC}" : "\u{1F91D}",
    ...
    const parseOverview = () => {
      ...
      const nameNode = subHeader == null ? void 0 : subHeader.querySelector(".large");
      const ownerNode = subHeader == null ? void 0 : subHeader.querySelector(".italic");
      ...
      return {
        title: cleanText2((nameNode == null ? void 0 : nameNode.textContent) || "Friendly League"),
    ...
    const renderOverviewCard = (overview) => {
      ...
      TmHeroCard.mount(wrap, {
        ...
        slots: {
          ...
          title: escapeHtml4(overview.title),
          main: `
                    <div class="tmvu-fl-byline">${overview.ownerHtml}</div>
                    <div class="tmvu-fl-overview-metrics">
                        ${hasValue(overview.rank) ? metricHtml({ label: "Your Rank", value: `#${escapeHtml4(overview.rank)}`, tone: "overlay", size: "md", align: "center" }) : ""}
                        ${hasValue(overview.points) ? metricHtml({ label: "Points", value: escapeHtml4(overview.points), tone: "overlay", size: "md", align: "center" }) : ""}
                        ${hasValue(overview.goals) ? metricHtml({ label: "Goals", value: escapeHtml4(overview.goals), tone: "overlay", size: "md", align: "center" }) : ""}
                    </div>
                `,
    ...
    const renderChatCard = (chat) => {
      ...
      if (chat.actionNode) {
        ...
        actionMount == null ? void 0 : actionMount.appendChild(button);
      }
      return card;
    };
