...
` }));
  ...
  (function() {
    ...
    const STYLE_ID3 = "tmvu-fin-wages-style";
    ...
    const extractPlayerId = (anchor) => {
      var _a;
      if (!anchor) return "";
      return cleanText(anchor.getAttribute("player_link")) || (((_a = cleanText(anchor.getAttribute("href")).match(/\/players\/(\d+)\//)) == null ? void 0 : _a[1]) || "");
    };
      ...
      return Array.from(table.querySelectorAll("tr")).slice(1).map((row) => {
        ...
        if (cells.length >= 3 && /total/i.test(row.textContent || "")) {
          return {
            ...
            isTotal: true,
            label: "Total",
            weekly: parseMoney(((_a = cells[cells.length - 2]) == null ? void 0 : _a.textContent) || "0"),
            season: parseMoney(((_b = cells[cells.length - 1]) == null ? void 0 : _b.textContent) || "0")
          };
        }
        if (kind === "players" && cells.length >= 5) {
          ...
          return {
            kind,
            isTotal: false,
            order: cleanText((_c = cells[0]) == null ? void 0 : _c.textContent),
            name: cleanText((playerAnchor == null ? void 0 : playerAnchor.textContent) || nameCell.textContent),
            nameHtml: `${(playerAnchor == null ? void 0 : playerAnchor.outerHTML) || escapeHtml3(cleanText(nameCell.textContent))}${countryAnchor ? ` ${countryAnchor.outerHTML}` : ""}`,
            playerId: extractPlayerId(playerAnchor),
            age: cleanText((_d = cells[2]) == null ? void 0 : _d.textContent),
            weekly: parseMoney(((_e = cells[3]) == null ? void 0 : _e.textContent) || "0"),
            season: parseMoney(((_f = cells[4]) == null ? void 0 : _f.textContent) || "0")
          };
        }
        if (kind === "staff" && cells.length >= 5) {
          return {
            kind,
            isTotal: false,
            role: cleanText((_g = cells[0]) == null ? void 0 : _g.textContent),
            name: cleanText((_h = cells[1]) == null ? void 0 : _h.textContent),
            age: cleanText((_i = cells[2]) == null ? void 0 : _i.textContent),
            weekly: parseMoney(((_j = cells[3]) == null ? void 0 : _j.textContent) || "0"),
            season: parseMoney(((_k = cells[4]) == null ? void 0 : _k.textContent) || "0")
          };
        }
        return null;
      }).filter(Boolean);
        ...
        slots: {
          ...
          footer: `
                    <div class="tmvu-fin-hero-metrics">
                        ${metricHtml({ label: `Players / ${periodLabel}`, value: escapeHtml3(formatMoney(playersValue)), tone: "overlay", size: "md" })}
                        ${metricHtml({ label: `Staff / ${periodLabel}`, value: escapeHtml3(formatMoney(staffValue)), tone: "overlay", size: "md" })}
                        ${metricHtml({ label: `Total / ${periodLabel}`, value: escapeHtml3(formatMoney(playersValue + staffValue)), tone: "overlay", size: "lg" })}
                    </div>
                `
        }
      });
        ...
        if (row.isTotal) {
          return `
                                    <tr class="tmvu-fin-wages-total">
                                        ...
                                        <td>${escapeHtml3(formatMoney(getPeriodValue(row, payPeriod)))}</td>
                                    </tr>
                                `;
        }
        return `
                                <tr>
                                    <td data-align="center">${escapeHtml3(kind === "players" ? row.order : row.role || "")}</td>
                                    <td data-align="left">${kind === "players" ? `<span class="tmvu-fin-wages-name">${row.nameHtml}</span>` : escapeHtml3(row.name)}</td>
                                    <td data-align="center">${escapeHtml3(row.age || "-")}</td>
                                    <td>${escapeHtml3(formatMoney(getPeriodValue(row, payPeriod)))}</td>
                                </tr>
                            `;
      }).join("")}
    ...
    const renderTableCard = (tabsData, state, onChange) => {
      ...
      const refs = TmUI.render(wrap, `
            <tm-card data-title="Wage Ledger" data-icon="\u{1F4B8}">
    ...
    const renderSummaryCard = (title, icon, summary, payPeriod) => {
      ...
      const periodLabel = getPeriodLabel(payPeriod);
      TmUI.render(wrap, `
            <tm-card data-title="${escapeHtml3(title)}" data-icon="${escapeHtml3(icon)}">
                <div class="tmvu-fin-highlights">
                    ${metricHtml({ label: "Headcount", value: escapeHtml3(String(summary.headcount)), layout: "row", tone: "muted", size: "sm" })}
                    ${metricHtml({ label: `${periodLabel} Total`, value: escapeHtml3(formatMoney(getSummaryTotal(summary, payPeriod))), layout: "row", tone: "muted", size: "sm" })}
                    ${metricHtml({ label: `Avg / ${periodLabel}`, value: escapeHtml3(formatMoney(getSummaryAverage(summary, payPeriod))), layout: "row", tone: "muted", size: "sm" })}
                </div>
            </tm-card>
        `);
      return wrap.firstElementChild || wrap;
    ...
    const render = () => {
      ...
      const paint = () => {
        ...
        sideCol.appendChild(renderSummaryCard("Players Payroll", "\u{1F465}", playersSummary, state.payPeriod));
        sideCol.appendChild(renderSummaryCard("Staff Payroll", "\u{1F9ED}", staffSummary, state.payPeriod));
      };
      paint();
